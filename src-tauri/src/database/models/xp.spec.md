...
| 定数                        | 値   | 意味                                                                 |
| --------------------------- | ---- | -------------------------------------------------------------------- |
| `STREAK_BONUS_PERCENT`      | 10   | `with_streak_bonus` 用、1 日あたりのボーナス %                       |
| `MAX_STREAK_BONUS_PERCENT`  | 100  | `with_streak_bonus` の上限 %                                         |
| `STREAK_BONUS_CAP_DAYS`     | 10   | `XpBreakdown::calculate` の合計に乗算するストリーク日数の上限（10 日） |
...
で算出される（最大 +10%）。`with_streak_bonus` ヘルパは別系統（個別アクションの
ボーナス計算用）で、`MAX_STREAK_BONUS_PERCENT` までスケールする。
...
- `with_streak_bonus(base_xp: i32, streak: i32) -> i32`
  個別アクションの XP に対して、ストリーク日数に応じた % ボーナスを乗せて返す。
...
- `tests::test_streak_bonus` — `with_streak_bonus` の境界値

...
### TC-002: First run without startup sync sleeps when no history
...
- Given: `is_first_run=false`, `sync_on_startup=true`, `last_sync_at=None`
- When: `decide_action(&inputs)`
- Then: `SchedulerAction::RunSync` — `sync_on_startup=true` callers want
  syncs to happen, so the recovery path doesn't suppress.

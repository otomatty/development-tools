...
> ⚠️ **注意**: 本書は当初構想時の要件定義書であり、現行プロダクトの仕様とは大きく異なります。**CLIツール関連の機能は完全に廃止されました**（GUIからCLIツールを実行する機能、および `tools/` ディレクトリ配下の独立CLIツール群の同梱を含む）。現行プロダクトはGitHub連携によるゲーミフィケーションを中心としたデスクトップアプリケーションです。以下の本文はあくまで当初構想の記録として残しています。
>
> 最終更新日: 2026年6月16日（CLI機能の完全廃止を反映）

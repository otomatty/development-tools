...
export const Button = ({ variant = 'primary', size = 'md', disabled, fullWidth, isLoading, leftIcon, rightIcon, children, className, ...rest }: ButtonProps) => {
  const isDisabled = disabled || isLoading;
  const widthClass = fullWidth ? 'w-full' : '';
  const combinedClass = `${baseClasses} ${variantClasses[variant]} ${sizeClasses[size]} ${widthClass} ${className || ''}`.trim();

  return (
    <button type={rest.type || 'button'} className={combinedClass} disabled={isDisabled} {...rest}>
      {isLoading && (

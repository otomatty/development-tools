# Large File Finder

閾値以上の行数を持つファイルを検出し、行数降順で表示する CLI ツールです。

## 特徴

- **`.gitignore` 自動適用**: リポジトリの `.gitignore` パターンを自動で除外
- **行数降順ソート**: 最も大きいファイルから順に表示
- **上位 N 件表示**: `--top` オプションで上位 N 件のみ表示可能
- **複数出力形式**: テキスト（カラー）と JSON 形式をサポート
- **プリセット機能**: 言語別の除外パターン（Rust, Node.js, Python, Go）
- **自動検出**: プロジェクトファイル（Cargo.toml, package.json 等）から言語を自動検出
- **設定ファイル**: `.largefile.toml` でプロジェクト固有の設定を定義可能

## インストール

```bash
cd tools/large-file-finder
cargo build --release
```

## 使い方

### 基本使用

```bash
# カレントディレクトリで500行以上のファイルを検出（デフォルト）
large-file-finder

# 指定ディレクトリをスキャン
large-file-finder ~/projects/my-app

# 閾値を変更（300行以上）
large-file-finder --min-lines 300

# 上位10件のみ表示
large-file-finder --top 10
```

### 出力形式

```bash
# テキスト出力（カラー表示）
large-file-finder --output text

# JSON出力（デフォルト）
large-file-finder --output json
```

### プリセット機能

```bash
# Rustプリセットを使用
large-file-finder --preset rust

# 複数プリセットを組み合わせ
large-file-finder --preset rust,node

# 自動検出を無効化
large-file-finder --no-auto-detect
```

利用可能なプリセット:

- `common`: 一般的な除外パターン（.git, node_modules, target 等）
- `rust`: Rust プロジェクト用（target/, \*.rlib 等）
- `node`: Node.js プロジェクト用（node_modules/, dist/ 等）
- `python`: Python プロジェクト用（**pycache**/, .venv/ 等）
- `go`: Go プロジェクト用（vendor/, \*.exe 等）

### 追加の除外パターン

```bash
# 追加で除外するパターンを指定
large-file-finder --exclude "generated/**,*.min.js"
```

## 設定ファイル

プロジェクトルートに `.largefile.toml` を配置することで、プロジェクト固有の設定を定義できます。

### 設定例

```toml
# 除外パターン（.gitignore に加えて適用）
exclude = [
    "generated/**",
    "*.min.js",
    "vendor/**"
]

# 最小行数の閾値（デフォルト: 500）
min_lines = 300

# プリセット設定
[presets]
# 自動検出を有効化（デフォルト: true）
auto_detect = true
# 手動でプリセットを指定（自動検出より優先）
# manual = ["rust", "node"]
```

### 設定の優先順位

1. CLI オプション（最高優先）
2. 設定ファイル（`.largefile.toml`）
3. 自動検出
4. デフォルト値

## 出力例

### テキスト出力

```
→ Scanning directory: /Users/user/projects/my-app
→ Config: .largefile.toml found
→ Min lines threshold: 300
→ Showing top 10 files
→ Presets: common, rust (auto-detected)
→ Excluding: 16 patterns
→ Using .gitignore patterns

✓ Found 2029 files to analyze

🔍 Large File Finder
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

→ Scanning: /Users/user/projects/my-app
→ Min lines: 300
→ Excluding: .gitignore patterns

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Found 5 files with 300+ lines:

    1. 1,927 lines  src/database/models.rs
    2.   854 lines  src/github/client.rs
    3.   821 lines  src/commands/github.rs
    4.   741 lines  docs/prd/home-gamification.md
    5.   512 lines  src/components/contribution_graph.rs

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Total: 5 files, 4,855 lines combined
Scanned: 2,029 files
```

### JSON 出力

```json
{
  "scan_directory": "/Users/user/projects/my-app",
  "min_lines": 300,
  "presets": ["common", "rust"],
  "files": [
    {
      "path": "src/database/models.rs",
      "lines": 1927
    },
    {
      "path": "src/github/client.rs",
      "lines": 854
    }
  ],
  "total_files_scanned": 2029,
  "total_lines": 4855
}
```

## CLI オプション

| オプション         | 短縮形 | デフォルト | 説明                               |
| ------------------ | ------ | ---------- | ---------------------------------- |
| `--min-lines`      | `-m`   | `500`      | 大きいファイルとみなす最小行数     |
| `--top`            | `-t`   | なし       | 上位 N 件のみ表示                  |
| `--output`         | `-o`   | `json`     | 出力形式 (`text` または `json`)    |
| `--preset`         | `-p`   | なし       | プリセット指定（カンマ区切り）     |
| `--no-auto-detect` |        | `false`    | プリセット自動検出を無効化         |
| `--exclude`        | `-e`   | なし       | 追加の除外パターン（カンマ区切り） |

## ユースケース

### リファクタリング対象の特定

```bash
# 1000行以上の巨大ファイルを見つける
large-file-finder --min-lines 1000 --output text
```

### コードレビュー前のチェック

```bash
# 新規追加されたファイルの肥大化をチェック
large-file-finder --min-lines 300 --top 10
```

### CI/CD での品質チェック

```bash
# JSON出力で閾値超えファイル数をチェック
large-file-finder --min-lines 500 --output json | jq '.files | length'
```

## 技術詳細

- **ファイルスキャン**: `ignore` crate を使用し、`.gitignore`、`.git/info/exclude`、グローバル gitignore を自動的に考慮
- **バイナリファイル検出**: ファイル先頭 512 バイトを読み取り、NULL バイトが含まれる場合はスキップ
- **パフォーマンス**: `BufReader`を使用した効率的な行カウント

## ライセンス

MIT
